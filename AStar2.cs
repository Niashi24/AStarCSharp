namespace NS.AStar2
{
	...
	public interface Graph<TNode>
	{
        ...
        bool NodesEqual(TNode a, TNode b);
	}
	
	public static class AStar
	{
		public static (List<TNode>, int) AStarSearch<TNode>(Graph<TNode> graph, IEqualityComparer<TNode> nodeComparer, TNode start, TNode end)
		{
			Dictionary<TNode, int> G = new(nodeComparer) {{start, 0}};
			Dictionary<TNode, int> F = new(nodeComparer) {{start, graph.Heuristic(start, end)}};
